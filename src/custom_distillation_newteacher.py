...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, UNet2DConditionModel, AutoPipelineForText2Image, DPMSolverMultistepScheduler
...
def main():
    ...
    nt_text_encoder_cls_one = import_model_class_from_model_name_or_path(new_teacher_model, new_teacher_revision)
    nt_text_encoder_cls_two = import_model_class_from_model_name_or_path(
    ...
    nt_vae = AutoencoderKL.from_pretrained(
        nt_vae_path,
        subfolder="vae",
        revision=new_teacher_revision,
        variant=new_teacher_variant,
    )
    nt_unet = UNet2DConditionModel.from_pretrained(new_teacher_model, subfolder="unet", revision=new_teacher_revision, variant=new_teacher_variant)
    nt_vae.requires_grad_(False)
    nt_text_encoder_one.requires_grad_(False)
    nt_text_encoder_two.requires_grad_(False)
    nt_unet.requires_grad_(False)
    nt_weight_dtype = torch.float16
    ...
    def preprocess_train(examples):
        ...
        examples["crop_top_lefts"] = examples["pixel_values"]  #all_images
    ...
    nt_unet.to(accelerator.device, dtype=weight_dtype)
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers("distillation-swish", config=vars(args), init_kwargs={"wandb":{"name":"newteacher_"+args.pretrained_model_name_or_path.split("/")[-1]+"_"+args.unet_config_name+"_"+str(timenow)}})
    ...
    logger.info(f"  Parameter Count: {sum(p.numel() for p in unet.parameters())}")
    global_step = 0
    first_epoch = 0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                model_input = batch["pixel_values"].to(accelerator.device)
                ...
                loss = args.lambda_sd * loss_sd + args.lambda_kd_output * loss_kd_output_original_teacher + args.lambda_kd_output * loss_kd_output_new_teacher + args.lambda_kd_feat * loss_kd_feat
